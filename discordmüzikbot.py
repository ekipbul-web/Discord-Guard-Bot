...
async def play(ctx, *, query):
    ...
    try:
        ...
        data = await loop.run_in_executor(None, lambda: ytdl.extract_info(f"ytsearch:{query}" if not query.startswith('http') else query, download=False))
        ...
        if 'entries' in data and len(data['entries']) > 1:
            ...
            await msg.edit(content=f"✅ **{len(data['entries'])}** şarkı sıraya eklendi!")
        else:
...
# Yardım
@bot.command(name='yardim', aliases=['help', 'h'])
