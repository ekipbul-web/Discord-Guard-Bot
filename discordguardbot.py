...
# Streamer ayarları
STREAMER_KATEGORILER = ["Streamer", "Yayın", "Stream", "Yayın Odaları"]
...
def streamer_oda_kontrol(kanal):
    ...
    if kanal and kanal.category:
        for kat in STREAMER_KATEGORILER:
            if kat.lower() in kanal.category.name.lower():
                return True
    return False
                        ...
                        if yayin_data.get("baslangic"):
                            ...
                            if (datetime.now() - baslangic).total_seconds() > AFK_YAYIN_SURE * 60:
                                if member.voice and not member.voice.self_stream and not member.voice.self_video:
                                    # AFK yayın tespit edildi
                                    log_kanal = get(guild.text_channels, name=MOD_LOG_KANALI)
                                    if log_kanal:
                                        await log_kanal.send(f"⚠️ {member.mention} **{AFK_YAYIN_SURE} dakikadır** yayın açmadan bekliyor!")
...
async def streamlock(ctx):
    ...
    await ctx.send(f"🔒 **{kanal.name}** streamer odası kilitlendi! Sadece yayıncı konuşabilir.")
...
async def streamunlock(ctx):
    ...
    await ctx.send(f"🔓 **{kanal.name}** streamer odası kilidi açıldı!")
...
async def streamceza(ctx, member: discord.Member, *, reason: str = "Sebep belirtilmedi"):
    ...
    if not ceza_rolu:
        try:
            ...
            await ctx.send("✅ Streamer Cezalı rolü oluşturuldu!")
