...
STREAMER_CEZA_ROLU = "Streamer Cezalı"
AFK_YAYIN_SURE = 30
...
def veri_yukle():
    ...
    return {
        ...
        "streamer_cezalar": [], "oda_sahipleri": {}, "yayin_kayit": {},
        "yetki_cekme": {}
    }
...
def streamer_oda_kontrol(kanal):
    """Kanal streamer odası mı?"""
    if kanal:
        # Kanal isminde "streamer" geçiyor mu?
        if "streamer" in kanal.name.lower():
            return True
        # Kategori isminde "SES KANALLARI" geçiyor mu?
        if kanal.category and "SES KANALLARI" in kanal.category.name:
            return True
    return False
...
async def on_ready():
    ...
    print(f"🎥 Streamer odaları: 'streamer' isimli veya 'SES KANALLARI' kategorisindeki kanallar")
...
async def afk_yayin_kontrol():
    """AFK yayınları kontrol eder"""
    for guild in bot.guilds:
        for vc in guild.voice_channels:
            if streamer_oda_kontrol(vc):
                for member in vc.members:
                    if not member.bot:
                        veri = veri_yukle()
                        yayin_data = veri["yayin_kayit"].get(str(member.id), {})
                        
                        if yayin_data.get("baslangic"):
                            baslangic = datetime.fromisoformat(yayin_data["baslangic"])
                            if (datetime.now() - baslangic).total_seconds() > AFK_YAYIN_SURE * 60:
                                if member.voice and not member.voice.self_stream and not member.voice.self_video:
                                    log_kanal = get(guild.text_channels, name=MOD_LOG_KANALI)
                                    if log_kanal:
                                        await log_kanal.send(f"⚠️ {member.mention} **{AFK_YAYIN_SURE} dakikadır** yayın açmadan streamer odasında bekliyor!")
...
# -------------------- SES OLAYLARI - STREAMER KORUMA --------------------
@bot.event
async def on_voice_state_update(member, before, after):
    """Streamer koruma sistemi"""
    if member.bot: return
    
    # Streamer odasına katıldı
    if after.channel and streamer_oda_kontrol(after.channel):
        veri = veri_yukle()
        veri["yayin_kayit"][str(member.id)] = {
            "baslangic": datetime.now().isoformat(),
            "kanal": after.channel.id
        }
        veri_kaydet(veri)
        
        # Cezalı kontrolü
        ceza_rolu = get(member.guild.roles, name=STREAMER_CEZA_ROLU)
        if ceza_rolu and ceza_rolu in member.roles:
            try:
                await member.move_to(None)
                try: await member.send("🚫 Streamer cezanız olduğu için yayın odalarına katılamazsınız!")
                except: pass
            except: pass
    
    # Streamer odasından ayrıldı
    if before.channel and streamer_oda_kontrol(before.channel):
        veri = veri_yukle()
        if str(member.id) in veri["yayin_kayit"]:
            del veri["yayin_kayit"][str(member.id)]
            veri_kaydet(veri)

# -------------------- STREAMER KOMUTLARI --------------------
@bot.command(name='streamlock')
@commands.has_permissions(manage_channels=True)
async def streamlock(ctx):
    """Streamer odasını kilitler - sadece yayıncı konuşabilir"""
    if not ctx.author.voice or not ctx.author.voice.channel:
        return await ctx.send("❌ Önce bir ses kanalına katıl!")
    
    if not streamer_oda_kontrol(ctx.author.voice.channel):
        return await ctx.send("❌ Bu bir streamer odası değil!")
    ...
    embed = discord.Embed(title="🔒 STREAMER ODA KİLİTLENDİ", description=f"**{kanal.name}** odasında sadece yayıncı konuşabilir!", color=0xFF0000)
    embed.add_field(name="👤 Yayıncı", value=ctx.author.mention)
    await ctx.send(embed=embed)
    await log_gonder(ctx.guild, embed)

@bot.command(name='streamunlock')
@commands.has_permissions(manage_channels=True)
async def streamunlock(ctx):
    """Streamer odası kilidini açar"""
    if not ctx.author.voice or not ctx.author.voice.channel:
        return await ctx.send("❌ Önce bir ses kanalına katıl!")
    ...
    embed = discord.Embed(title="🔓 STREAMER ODA AÇILDI", description=f"**{kanal.name}** odası kilidi açıldı!", color=0x00FF00)
    embed.add_field(name="👤 Açan", value=ctx.author.mention)
    await ctx.send(embed=embed)
    await log_gonder(ctx.guild, embed)

@bot.command(name='streamceza')
@commands.has_permissions(manage_roles=True)
async def streamceza(ctx, member: discord.Member, *, reason: str = "Sebep belirtilmedi"):
    """Streamer cezası verir"""
    ceza_rolu = get(ctx.guild.roles, name=STREAMER_CEZA_ROLU)
    
    if not ceza_rolu:
        try:
            ceza_rolu = await ctx.guild.create_role(
                name=STREAMER_CEZA_ROLU,
                color=0xFF0000,
                hoist=True,
                reason="Streamer ceza rolü"
            )
            await ctx.send("✅ Streamer Cezalı rolü otomatik oluşturuldu!")
        except: return await ctx.send("❌ Rol oluşturulamadı! Lütfen 'Streamer Cezalı' rolünü manuel oluşturun.")
    
    if ceza_rolu in member.roles:
        return await ctx.send(f"❌ {member.mention} zaten streamer cezalısı!")
    
    await member.add_roles(ceza_rolu)
    
    veri = veri_yukle()
    veri["streamer_cezalar"].append({
    ...
    embed = discord.Embed(title="🎥 STREAMER CEZASI", description=f"**{member.name}** streamer cezası aldı!", color=0xFF0000, timestamp=datetime.now())
    ...
    embed.set_thumbnail(url=member.display_avatar.url)
    embed.set_footer(text="Kross Guard • Streamer Ceza")
    await ctx.send(embed=embed)
    await log_gonder(ctx.guild, embed)

@bot.command(name='streamaf')
@commands.has_permissions(manage_roles=True)
async def streamaf(ctx, member: discord.Member):
    """Streamer affı - cezayı kaldırır"""
    ceza_rolu = get(ctx.guild.roles, name=STREAMER_CEZA_ROLU)
    if not ceza_rolu: return await ctx.send("❌ Streamer ceza rolü bulunamadı!")
    if ceza_rolu not in member.roles: return await ctx.send(f"❌ {member.mention} streamer cezalısı değil!")
    
    await member.remove_roles(ceza_rolu)
    
    embed = discord.Embed(title="✅ STREAMER AFFI", description=f"**{member.name}** affedildi!", color=0x00FF00, timestamp=datetime.now())
    embed.add_field(name="🛡️ Yetkili", value=ctx.author.mention, inline=True)
    embed.set_thumbnail(url=member.display_avatar.url)
    embed.set_footer(text="Kross Guard • Streamer Af")
    await ctx.send(embed=embed)
    await log_gonder(ctx.guild, embed)

@bot.command(name='cezalilar')
async def cezalilar(ctx):
    """Streamer cezalı listesini gösterir"""
    ceza_rolu = get(ctx.guild.roles, name=STREAMER_CEZA_ROLU)
    if not ceza_rolu: return await ctx.send("❌ Streamer ceza rolü bulunamadı!")
    
    uyeler = ceza_rolu.members
    if not uyeler: return await ctx.send("✅ Hiç streamer cezalısı yok!")
    
    embed = discord.Embed(title="🚫 Streamer Cezalılar", description="\n".join([f"• {m.mention}" for m in uyeler[:20]]), color=0xFF0000)
    embed.set_footer(text=f"Toplam: {len(uyeler)} cezalı")
    await ctx.send(embed=embed)

@bot.command(name='yetkicek')
@commands.has_permissions(administrator=True)
async def yetkicek(ctx, member: discord.Member, *, reason: str = "Sebep belirtilmedi"):
    """Geçici yetki çekimi - tüm rollerini alır, kaydeder"""
    if member.top_role >= ctx.author.top_role: return await ctx.send("❌ Yetkin yetmez!")
    
    veri = veri_yukle()
    roller = [r.id for r in member.roles if r.name != "@everyone"]
    
    if "yetki_cekme" not in veri: veri["yetki_cekme"] = {}
    veri["yetki_cekme"][str(member.id)] = {"roller": roller, "tarih": datetime.now().isoformat()}
    veri_kaydet(veri)
    ...
    embed = discord.Embed(title="⚠️ YETKİ ÇEKİLDİ", description=f"**{member.name}** tüm yetkileri alındı!", color=0xFFA500, timestamp=datetime.now())
    embed.add_field(name="👤 Kişi", value=member.mention, inline=False)
    embed.add_field(name="🛡️ Yetkili", value=ctx.author.mention, inline=True)
    embed.add_field(name="📝 Sebep", value=reason, inline=True)
    embed.set_footer(text="Kross Guard • Yetki Çekme")
    await ctx.send(embed=embed)
    await log_gonder(ctx.guild, embed)

@bot.command(name='yetkiver')
@commands.has_permissions(administrator=True)
async def yetkiver(ctx, member: discord.Member):
    """Çekilen yetkileri geri verir"""
    veri = veri_yukle()
    yetki_data = veri.get("yetki_cekme", {}).get(str(member.id))
    
    if not yetki_data:
        return await ctx.send("❌ Bu kullanıcının çekilmiş yetkisi yok!")
    
    for rol_id in yetki_data["roller"]:
        rol = ctx.guild.get_role(rol_id)
        if rol:
            try: await member.add_roles(rol)
            except: pass
    
    del veri["yetki_cekme"][str(member.id)]
    veri_kaydet(veri)
    
    await ctx.send(f"✅ {member.mention} yetkileri geri verildi!")

@bot.command(name='odakontrol')
@commands.has_permissions(manage_channels=True)
async def odakontrol(ctx):
    """Tüm streamer odalarını kontrol eder"""
    embed = discord.Embed(title="🎥 Streamer Odaları", color=0x5865F2, timestamp=datetime.now())
    
    for vc in ctx.guild.voice_channels:
        if streamer_oda_kontrol(vc):
            uyeler = [m.mention for m in vc.members if not m.bot]
            yayin_durum = "🔴 Yayın Yok"
            
            for m in vc.members:
                if m.voice and (m.voice.self_stream or m.voice.self_video):
                    yayin_durum = f"🟢 {m.mention} yayında!"
                    break
            
            embed.add_field(
                name=f"{vc.name} ({len(uyeler)} kişi)",
                value=f"{yayin_durum}\nÜyeler: {', '.join(uyeler) if uyeler else 'Boş'}",
                inline=False
            )
    
    if not embed.fields:
        embed.description = "Hiç streamer odası bulunamadı!"
    
    await ctx.send(embed=embed)

@bot.command(name='streamgecmis')
@commands.has_permissions(manage_messages=True)
async def streamgecmis(ctx, member: discord.Member):
    """Streamer ceza geçmişini gösterir"""
    veri = veri_yukle()
    cezalar = [c for c in veri.get("streamer_cezalar", []) if c["kullanici_id"] == member.id]
    
    if not cezalar:
        return await ctx.send(f"✅ {member.mention} hiç streamer cezası almamış!")
    
    embed = discord.Embed(title=f"🎥 {member.display_name} Streamer Geçmişi", color=0xFF0000, timestamp=datetime.now())
    embed.add_field(name="📊 Toplam Ceza", value=str(len(cezalar)), inline=True)
    
    for c in cezalar[-5:]:
        embed.add_field(name=f"📝 {c['sebep'][:50]}", value=f"👤 {c['yetkili']}", inline=False)
    
    embed.set_thumbnail(url=member.display_avatar.url)
    embed.set_footer(text="Kross Guard • Streamer Geçmiş")
    await ctx.send(embed=embed)
...
async def gecmis(ctx, member: discord.Member):
    ...
    embed.add_field(name="⚠️ Uyarı", value=str(uyari_data.get("sayi", 0)), inline=True)
    embed.add_field(name="🚫 Reklam", value=str(veri.get("reklam_kayit", {}).get(uid, 0)), inline=True)
    embed.add_field(name="⚠️ Küfür", value=str(veri.get("kufur_kayit", {}).get(uid, 0)), inline=True)
    embed.add_field(name="🎥 Streamer Ceza", value=str(len([c for c in veri.get("streamer_cezalar", []) if c["kullanici_id"] == member.id])), inline=True)
    
    mute_data = veri.get("mute_kayit", {}).get(uid, {})
    embed.add_field(name="🔇 Mute", value=f"🔇 Susturulmuş ({mute_data.get('sure', '?')} dk)" if mute_data else "✅ Temiz", inline=True)
...
async def yardim(ctx):
    embed = discord.Embed(
        ...
        description="Gelişmiş Güvenlik • Moderasyon • Streamer",
        color=0x5865F2,
        timestamp=datetime.now()
    )
    ...
    embed.add_field(name="🎥 **Streamer**", value="`.streamlock` `.streamunlock` `.streamceza` `.streamaf` `.cezalilar` `.yetkicek` `.yetkiver` `.odakontrol` `.streamgecmis`", inline=False)
...
if __name__ == "__main__":
    ...
    print("🎥 Streamer Koruma | 🔵 Verifiziert | 📋 Geçmiş | 🎉 Çekiliş")
    print("📁 Streamer odaları: 'streamer' isimli veya 'SES KANALLARI' kategorisi")
